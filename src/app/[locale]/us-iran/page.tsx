...
export default function UsIranPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch("/api/events?region=us-iran&page_size=50");
        if (res.ok) {
  ...
  const eventTypeColors: Record<string, string> = {
    ...
    strategic_development: "bg-blue-500",
    naval_clash: "bg-cyan-500",
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="container mx-auto px-4 py-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold tracking-tight">🇺🇸🇮🇱🇮🇷 {t('regions.usIran')}</h1>
        </div>
        ...
        <Tabs defaultValue="events" className="w-full">
          ...
          <TabsContent value="map" className="mt-6">
            <RegionMap events={events} center={[35.0, 50.0]} zoom={4} />
          </TabsContent>
